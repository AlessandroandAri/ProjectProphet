...
```
```{r}
forecast_o3$yhat <- log(forecast_o3$yhat)
```
...
```
```{r}
test_daily_o3$y <- log(test_daily_o3$y)
```
...
```{r}
last_date <- max(training_data$date)
prediction_period <- seq(last_date + 1, length.out = 365, by = "day")
prediction_data <- data.frame(date = prediction_period)

```

```{r}
# Melatih model
predic_svr_train1 <- predict(modelsvr_so2, newdata = prediction_data)
...
#Model Prophet
```{r}
ggplot(data = vis_so2, aes(x =first_date_month , y = hasil_prediksi, color = "SO2"))+ geom_line() +
geom_line(data = vis_pm10, aes(x =first_date_month , y = hasil_prediksi, color = "PM10")) +
geom_line(data = vis_no2, aes(x =first_date_month , y = hasil_prediksi, color = "NO2")) +
geom_line(data = vis_co, aes(x =first_date_month , y = hasil_prediksi, color = "CO")) +
geom_line(data = vis_o3, aes(x =first_date_month , y = hasil_prediksi, color = "O3")) +
