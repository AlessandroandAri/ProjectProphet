...
final_model <- prophet(changepoint.prior.scale = 0.05, 
  ...
  add_seasonality(name = "monthly", period = 30.5, fourier.order = 5)  %>% 
  fit.prophet(train_daily_so2) 
future_final <- make_future_dataframe(final_model, periods = nrow(test_daily_so2) + 1) 
...
prediction_test <- predict(final_model, test_daily_so2)
```
...
forecast_co$yhat <- ifelse(forecast_co$yhat < 0, 0, forecast_co$yhat)
...
forecast_final_co$yhat <- ifelse(forecast_final_co$yhat < 0, 0, forecast_final_co$yhat)
...
forecast_o3$yhat <- ifelse(forecast_o3$yhat < 0, 0, forecast_o3$yhat)
...
Berdasarkan gambar diatas bisa dilihat bahwa, tingkat partikel O3 untuk 2 tahun kedepan trennya terus mengalami penurunan. Setiap tahun bisa dilihat ada peningkatan O3 yang paling tinggi adalah di bulan Juli s/d Agustus.
...
Berdasarkan gambar tersebut bisa dilihat bahwa ada tren penurunan jumlah partikel O3 dibandingkan tahun-tahun sebelumnya
...
forecast_final_o3$yhat <- ifelse(forecast_final_o3$yhat < 0, 0, forecast_final_o3$yhat)
...
mean_MSE <- mean(model_performance1$MSE)
mean_MSE
```
```{r}
##Rata-rata nilai Akurasi
...
```{r}
##Rata-rata nilai Akurasi
mean_RMSE <- mean(model_performance1$RMSE)
mean_RMSE
```
```{r}
##Rata-rata nilai Akurasi
mean_MAE <- mean(model_performance1$MAE)
mean_MAE
```

```{r}
##Rata-rata nilai Akurasi
mean_MAPE <- mean(model_performance1$MAPE)
mean_MAPE
```
...
rata_rata_mse <- mean(model_performance2$MSE)
rata_rata_mse
```

```{r}
#rata-rata akurasi model SVR
rata_rata_rmse <- mean(model_performance2$RMSE)
rata_rata_rmse
```

```{r}
#rata-rata akurasi model SVR
rata_rata_mae <- mean(model_performance2$MAE)
rata_rata_mae
```

```{r}
#rata-rata akurasi model SVR
rata_rata_mape <- mean(model_performance2$MAPE)
rata_rata_mape
```
```{r}
##Nilai Hasil Evaluasi Model Prophet
...
```{r}
vis_so2 <- forecast_final %>% 
  mutate(first_date_month = floor_date(ds, unit = "month")) %>% 
  group_by(first_date_month) %>% 
  summarise(hasil_prediksi = sum (yhat))
vis_so2
```

```{r}
##Menghitung Rata-rata kenaikan tren
# Ambil kolom yang ingin dianalisis
prediksi <- vis_so2$hasil_prediksi

# Menghitung kenaikan
kenaikan <- diff(prediksi)

# Menghitung rata-rata kenaikan
rata_rata_kenaikan_tren <- mean(kenaikan)

print(paste("Rata-rata kenaikan dalam data prediksi:", rata_rata_kenaikan_tren))

```


```{r}
vis_pm10 <- forecast_finalpm10 %>% 
  mutate(first_date_month = floor_date(ds, unit = "month")) %>% 
  group_by(first_date_month) %>% 
  summarise(hasil_prediksi = sum (yhat))
vis_pm10
...
```
```{r}
##Mengonversi kolom tanggal menjadi kelas 'Date'
vis_so2$first_date_month <- as.Date(vis_so2$first_date_month)
vis_pm10$first_date_month <- as.Date(vis_pm10$first_date_month)
vis_no2$first_date_month <- as.Date(vis_no2$first_date_month)
vis_co$first_date_month <- as.Date(vis_co$first_date_month)
vis_o3$first_date_month <- as.Date(vis_o3$first_date_month)
```


##Visualisasi 5 Variabel
#Model Prophet
```{r}
ggplot(data = vis_so2, aes(x =first_date_month , y = hasil_prediksi, color = "#fff80e"))+ geom_line() +
geom_line(data = vis_pm10, aes(x =first_date_month , y = hasil_prediksi, color = "#0e8eff")) +
geom_line(data = vis_no2, aes(x =first_date_month , y = hasil_prediksi, color = "#30e97f")) +
geom_line(data = vis_co, aes(x =first_date_month , y = hasil_prediksi, color = "#0e0700")) +
geom_line(data = vis_o3, aes(x =first_date_month , y = hasil_prediksi, color = "#d19f00")) +
labs(title = "Visualisasi 5 Variabel Prediksi Prophet",
     x = "Tahun",
     y = "Jumlah Partikel (μg/m3)") +
theme_minimal() +
...
```{r}
svr_so2 <- data_test_svr1 %>% 
  mutate(first_date_month = floor_date(Tanggal, unit = "month")) %>% 
  group_by(first_date_month) %>% 
  summarise(hasil_prediksi = sum (predic_svr1))
svr_so2
```
```{r}
svr_pm10 <- data_test_svr2 %>% 
  mutate(first_date_month = floor_date(Tanggal, unit = "month")) %>% 
  group_by(first_date_month) %>% 
  summarise(hasil_prediksi = sum (predic_svr2))
svr_pm10
```

```{r}
svr_no2 <- data_test_svr3 %>% 
  mutate(first_date_month = floor_date(Tanggal, unit = "month")) %>% 
  group_by(first_date_month) %>% 
  summarise(hasil_prediksi = sum (predic_svr3))
svr_no2
```

```{r}
svr_co <- data_test_svr4 %>% 
  mutate(first_date_month = floor_date(Tanggal, unit = "month")) %>% 
  group_by(first_date_month) %>% 
  summarise(hasil_prediksi = sum (predic_svr4))
svr_co
```

```{r}
svr_o3 <- data_test_svr5 %>% 
  mutate(first_date_month = floor_date(Tanggal, unit = "month")) %>% 
  group_by(first_date_month) %>% 
  summarise(hasil_prediksi = sum (predic_svr5))
svr_o3
```

```{r}
##Mengonversi kolom tanggal menjadi kelas 'Date'
svr_so2$first_date_month <- as.Date(svr_so2$first_date_month)
svr_pm10$first_date_month <- as.Date(svr_pm10$first_date_month)
svr_no2$first_date_month <- as.Date(svr_no2$first_date_month)
svr_co$first_date_month <- as.Date(svr_co$first_date_month)
svr_o3$first_date_month <- as.Date(svr_o3$first_date_month)
```
